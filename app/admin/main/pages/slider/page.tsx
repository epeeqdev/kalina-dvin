...
import {CustomImage} from "@/app/components/image";
 import {ADD_SLIDES, SAVE_BUTTON, SLIDES} from "../../costants";
    ...
    return (
        <Controller control={localFormControl} render={({field}) => (
            <div>
                {
                    isModalOpen
                        ? <Modal classname="min-w-[50%] h-[90%]" title={ADD_SLIDES} onClose={() => {
                            setEditingItem(null)
                                        ...
                                        <Draggable id={item.id || item._id}>
                                            <div key={item?._id} className="border-2 border-gray-300 relative overflow-hidden">
                                                    ...
                                                    <div className="absolute top-[30%] left-0 pl-[30px] z-20">
                                                        <div className='text-[15px] text-red-600 mb-[5px] font-bold'>{getLanguage({am: item?.title?.am, ru: item?.title?.ru})}</div>
                                                        <div className='text-[5px] text-white mb-[5px] w-[60%]'>{getLanguage({am: item?.description?.am, ru: item?.description ?.ru})}</div>
                                                        {(!!item?.buttonText?.ru || !!item?.buttonText?.am) &&
                                                            <Button
                                                                title={{am: item?.buttonText?.am , ru: item?.buttonText?.ru }}
                                                                variant="alert"
                ...
                <div className="flex justify-end gap-2 relative">
                    <Alert
                        title={{am: "Ջնջել Սլայդը", ru: "Удалить Слайд"}}
                        isOpen={alertModal}
                        onAccept={() => {
                    ...
                    >
                        <p className="text-2xl font-bold">
                            {getLanguage({am: "Դուք վստահ եք որ ուզում եք ջնջել այս Սլայդը:", ru: "Вы уверены, что хотите удалить данный Слайд?"})}
                        </p>
                        <p className="text-gray-700">
                            {getLanguage({am: "Ջնջելուց հետո Սլայդը չի կարող վերականգնվել:", ru: "После удаления Слайд не возможно восстановить!"})}
                        </p>
                    </Alert>
                </div>
            </div>
